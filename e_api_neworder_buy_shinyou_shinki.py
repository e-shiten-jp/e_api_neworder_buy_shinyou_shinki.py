...
def func_login(int_p_no, my_url, str_userid, str_passwd, class_cust_property):
    ...
    if int_p_errno ==  0 and int_sResultCode == 0:    # ログインエラーでない場合
        ...
        if len(json_return.get('sUrlRequest')) > 0 :
            # 口座属性クラスに取得した値をセット
            class_cust_property.sZyoutoekiKazeiC = json_return.get('sZyoutoekiKazeiC')      # 8.譲渡益課税区分    1：特定  3：一般  5：NISA
            class_cust_property.sTokuteiKouzaKubunGenbutu = json_return.get('sTokuteiKouzaKubunGenbutu') # 12.特定口座区分現物    0：一般、1：特定源泉徴収なし、2：特定源泉徴収あり
            class_cust_property.sTokuteiKouzaKubunSinyou = json_return.get('sTokuteiKouzaKubunSinyou')  # 13.特定口座区分信用    0：一般、1：特定源泉徴収なし、2：特定源泉徴収あり。信用口座未開設の場合、0がセットされる。
            class_cust_property.sSinyouKouzaKubun = json_return.get('sSinyouKouzaKubun')    # 16.信用取引口座開設区分  0：未開設、1：開設
            class_cust_property.sHikazeiKouzaKubun = json_return.get('sHikazeiKouzaKubun')  # 21.非課税口座開設区分  0：未開設、1：開設  ※ＮＩＳＡ口座の開設有無を示す。
...
if bool_login :
    ...
    # 信用口座、開設チェック。
    if class_cust_property.sSinyouKouzaKubun == '1' :
        int_p_no = int_p_no + 1
        # 信用 新規 買い注文    引数：p_no、銘柄コード、市場（現在、東証'00'のみ）、執行条件、価格、株数、口座属性クラス
        dic_return = func_neworder_buy_sinyou_open(int_p_no,
                                                   my_sIssueCode,
                                                   my_sSizyouC,
                                                   my_sCondition,
                                                   my_sOrderPrice,
