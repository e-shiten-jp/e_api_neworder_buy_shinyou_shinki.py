...
def func_neworder_buy_sinyou_open(int_p_no,
    ...
    # 2:新規(制度信用6ヶ月) を指定
    str_sGenkinShinyouKubun = '2'   # 16.現金信用区分  0:現物、2:新規(制度信用6ヶ月)、4:返済(制度信用6ヶ月)、6:新規(一般信用6ヶ月)、8:返済(一般信用6ヶ月)。
...
##my_sIssueCode = '9861'     # 10.銘柄コード。実際の銘柄コードを入れてください。
my_sIssueCode = '7071'     # 10.銘柄コード。実際の銘柄コードを入れてください。
my_sSizyouC = '00'         # 11.市場。  00:東証   現在(2021/07/01)、東証のみ可能。
my_sCondition = '0'        # 13.執行条件。  0:指定なし、2:寄付、4:引け、6:不成。指し値は、0:指定なし。
my_sOrderPrice = '2250'     # 14.注文値段。  *:指定なし、0:成行、上記以外は、注文値段。小数点については、関連資料:「立花証券・e支店・API、REQUEST I/F、マスタデータ利用方法」の「2-12. 呼値」参照。
my_sOrderSuryou = '500'    # 15.注文数量。
